...
<head>
  ...
  <title>Admin | Gestion de Recrutement INSD</title>
...
<style>
	...
	#login-left{
		...
		display: flex;
		align-items: center;
		background: url(assets/img/recruitment-cove.jpg);
		...
		$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
			...
			success:function(resp){
				if(resp == 1){
					...
					$('#login-form').prepend('<div class="alert alert-danger">Email ou mot de passe incorrecte</div>')
					$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
				}
			}
		})
